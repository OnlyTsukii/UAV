...
class DroneController(Node):
    ...
    def goto_local_waypoint_test(self):
        waypoint = Waypoint(-1, TYPE_BODY_MOVE, MISSION_NONE, FRAME_BODY_NED, 0, -0.3, 0, 0, 0.3, 0)
    ...
    def goto_local_waypoint_test2(self):
        waypoint = Waypoint(-1, TYPE_BODY_MOVE, MISSION_NONE, FRAME_BODY_NED, 0, 0, 0, 0, 0, 0)
        point = PositionTarget()
        point.header = Header()
        point.header.stamp = self.get_clock().now().to_msg()   
        point.coordinate_frame = waypoint.frame 
    ...
    def process_waypoint(self):
        ...
        if self.last_sent_waypoint == None or self.reached_target() or self.has_takeoff:
            ...
            if not self.has_takeoff:
                ...
                if self.counter >= 100:
                    self.goto_local_waypoint_test2()
                else:
                    self.goto_local_waypoint_test()
                self.counter += 1
                return
