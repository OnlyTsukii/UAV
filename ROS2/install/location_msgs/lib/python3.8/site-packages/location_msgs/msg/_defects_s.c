...
bool location_msgs__msg__defects__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("location_msgs.msg._defects.Defects", full_classname_dest, 34) == 0);
  }
  location_msgs__msg__Defects * ros_message = _ros_message;
  {  // defect_id
  ...
  {  // defects
    ...
    if (!location_msgs__msg__DefectBox__Sequence__init(&(ros_message->defects), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create location_msgs__msg__DefectBox__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    location_msgs__msg__DefectBox * dest = ros_message->defects.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!location_msgs__msg__defect_box__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * location_msgs__msg__defects__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("location_msgs.msg._defects");
  ...
  {  // defects
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = location_msgs__msg__defect_box__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
